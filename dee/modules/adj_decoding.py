...
核心就是求最大团, 最大团的个数为event的个数, 最大团的交集即不同event的共享论元
给予最大团的每个团伸展, 得到event的entity
"""
...
def directed_trigger_graph_decode(
    ...
    if num_triggers == 1:  # one trigger mean one event exists
        ...
        if max_clique:
            trigger_combs = bron_kerbosch_pivoting_decode(fold_adj_mat, 2)  # find clips as combination difference
        else: 
...
def directed_trigger_graph_incremental_decode(
    ...
    Returns:
        list of combinations in tuple format
    """
    connections = build_single_element_connections(adj_mat, tuple_key=False)  # neigbor of each node
    combs = list()
    if num_triggers < 1:
        # guessing mode
        num_triggers = sum(len(val) > 0 for val in connections.values())
    ...
    if num_triggers == 1:
        ...
        for v in connections:  # trigger's neigbor
            comb = set()
            if len(connections[v]) > 0:
                comb.add(v)
