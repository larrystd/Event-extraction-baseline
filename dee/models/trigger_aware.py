...
class TriggerAwarePrunedCompleteGraph(LSTMMTL2CompleteGraphModel):
    ...
    def forward(
        ...
        # get doc span-level info reference DocArgRelInfo in trigger_aware.py
    ...
    def get_arg_combination_loss(
        ...
        # process diagonal condition, doc_arg_rel_info 包含图边连接的label
    ...
    def get_adj_mat_logits(self, hidden):  # return similarity score of mat
    ...
    def get_loss_on_doc(
        ...
        scores = self.get_adj_mat_logits(batch_span_context)  # span's similarity score mat

        # for each event type, get argument combination loss, one event-type can have multi combination
        # argument combination loss, calculated by comparing the biaffine output and the gold event SpanArgRelAdjMat
        ...
        if self.config.event_relevant_combination:
            raise RuntimeError("event_relevant_combination config is not supported yet")
        ...
        )  # doc_arg_rel_info 是挑选trigger形成的图
        ...
        for event_idx, event_label in enumerate(event_pred_list):
            ...
            # for every event, judge combs
            for comb in combs:
                event_table = self.event_tables[event_idx]
                gold_comb, _ = closest_match(comb, gold_combinations)  # get combinations
                ...
                )  # event-type argument, mutli-classify to role
