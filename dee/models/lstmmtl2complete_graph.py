...
class LSTMMTL2CompleteGraphModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        if ner_model is None:
            self.ner_model = LSTMCRFNERModel(config)  # default ner model
        else:
            self.ner_model = ner_model

        # all event tables, event table is the module that binary classify event type
    ...
    def get_event_cls_info(self, sent_context_emb, doc_fea, train_flag=True):
        """
        get the event type classification loss
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_arg_rel_info):
        ...
        # 0. get span representations with lstm
        ...
        else:  # based on event-relevant combination, attention between event representation and batch_span_context output
            for event_idx, event_label in enumerate(doc_fea.event_type_labels):
                event_table = self.event_tables[event_idx]
