...
import dee.models
from dee.event_template import get_event_template
...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ("train_file_name", "typed_test.json"),
        ...
        ("eval_batch_size", 8),
        ("learning_rate", 1e-3),
        ("use_lr_scheduler", False),  # 学习率调整策略, 随着epoch的进行学习率衰减
        ("lr_scheduler_step", 20),
        ("num_train_epochs", 20),
        ("no_cuda", False),
        ("local_rank", -1),  # 默认的local rank,-1意味不适用分布式训练
        ("seed", 99),
        ...
        (
            "bert_model",
            "~/bert-pretrained-models/bert-base-chinese",
    ...
    def __init__(self, **kwargs):  # kwargs是用户设置的dict
        super(DEETaskSetting, self).__init__(
            self.base_key_attrs, self.base_attr_default_pairs, **kwargs
        )
        if self.run_mode == "full":  # set file path
            self.train_file_name = "typed_test.json"
...
class DEETask(BasePytorchTask):
    ...
    def __init__(
        ...
        # event type template module, in event_type directory
        self.event_template = get_event_template(self.setting.event_type_template)

        # get entity BIO label
        self.entity_label_list = NERExample.get_entity_label_list(self.event_template)  # BIO label
        # event_type abstract
        self.event_type_fields_pairs = NERExample.get_event_type_fields_pairs(
            self.event_template
        )
        # build example dataset loader, used for load json and generate DEEExample for each doc
        ...
        ['O', 'B-StockCode', 'I-StockCode', 'B-StockAbbr', 'I-StockAbbr', 'B-CompanyName', 'I-CompanyName', 'B-OtherType', 'I-OtherType', 'B-EquityHolder', 'I-EquityHolder',
        """
        self.setting.tag_id2tag_name = {
            idx: name for idx, name in enumerate(self.entity_label_list)
        }
        """tag_id2tag_name tag idea

        0: 'O', 1: 'B-StockCode', 2: 'I-StockCode', 3: 'B-StockAbbr', 4: 'I-StockAbbr', 5: 'B-CompanyName' ...
            
        """
        self.setting.ent_type2id = {  # give all event role id
            event_type: idx
        ...
        # ['DCFEEModel', 'DEPPNModel', 'Doc2EDAGModel', 'GITModel', 'LSTMMTL2CompleteGraphModel', 'TriggerAwarePrunedCompleteGraph']
        if self.setting.use_bert:
            ...
            ner_model = None
        # self.setting.model_type: TriggerAwarePrunedCompleteGraph
        ...
        if self.setting.model_type in {"Doc2EDAG", "Doc2EDAGModel"}:
            self.model = Doc2EDAGModel(  # construct Doc2EDAG
                self.setting,
                self.event_type_fields_pairs,
                ner_model=ner_model,
            )
        elif self.setting.model_type in {"DCFEE", "DCFEEModel"}:
            self.model = DCFEEModel(  # construct DCFEE
                self.setting, self.event_type_fields_pairs, ner_model=ner_model
            )
        elif self.setting.model_type in supported_models:
            model_class = getattr(dee.models, self.setting.model_type)  # 获得class的属性参数
            self.model = model_class(
                self.setting, self.event_type_fields_pairs, ner_model=ner_model
            )  # 设置好self.model
        elif self.setting.model_type + "Model" in supported_models:
            model_class = getattr(dee.models, self.setting.model_type + "Model")  # TriggerAwarePrunedCompleteGraphModel
            self.model = model_class(
                self.setting, self.event_type_fields_pairs, ner_model=ner_model
            )
        else:
        ...
        all_trainable = []
        fixed = []
        for name, param in self.model.named_parameters():  # 返回各层中参数名称和数据
        ...
        self._decorate_model(parallel_decorate=parallel_decorate)  # 根据条件预备一些设置, 例如分布式训练设置, fp16 etc
        ...
        # use DEEArgRelFeature, feature_converter_func需要以example_loader_func 为参数, 包括构造graph作为图预测的label
        ...
        # load data, 导入数据, 并执行转化函数
        self._load_data(
            self.example_loader_func,  # load data func
            self.feature_converter_func,  # convert data func
    ...
    def train(self, save_cpt_flag=True, resume_base_epoch=None):
        ...
        Args:
            save_cpt_flag (bool, optional): _description_. Defaults to True.
            resume_base_epoch (_type_, optional): _description_. Defaults to None.
        """
        ...
        self.base_train(
            DEETask.get_loss_on_batch,  # forward
            kwargs_dict1={},
    ...
    def debug_display(self, doc_type, span_type, epoch, midout_dir):
        ...
        from dee.features import DEEArgRelFeature, DEEFeature, measure_event_table_filling
        from dee.utils import (
