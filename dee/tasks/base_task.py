...
class TaskSetting(object):
    ...
    def __init__(self, key_attrs, attr_default_pairs, **kwargs):   # 根据key_attrs, attr_default_pairs设置参数, **kwargs视作dict

        for key_attr in key_attrs:
            setattr(self, key_attr, kwargs.get(key_attr))  # 根据key_attrs和**kwargs设置参数

        for attr, val in attr_default_pairs:
            if attr in kwargs:
                setattr(self, attr, kwargs.get(attr))  # 优先根据kwargs设置
            else:
...
class BasePytorchTask(object):
    ...
    def __init__(self, setting, only_master_logging=False):
        ...
        if self.in_distributed_mode() and not dist.is_initialized():  # init pytorch distribute mode
    ...
    def load_example_feature_dataset(  # load dataset with load_example_func, and transform it with convert_to_feature_func, convert_to_dataset_func
        self,
        load_example_func,
        ...
        for epoch_idx in trange(
            ...
            for step, batch in step_batch_iter:
                ...
                if self.setting.global_grad_clip:
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), 5.0)  # 避免梯度爆炸, 给定选定的向量范数（ vector norm）来重新缩放梯度；以及裁剪超出预设范围的梯度值。
                ...
                if (step + 1) % self.setting.gradient_accumulation_steps == 0:
                    if self.setting.fp16 or self.setting.optimize_on_cpu:
                        ...
                        if is_nan:
                            ...
                            # 往往是loss随着每轮迭代越来越大，最终超过了浮点型表示的范围，就变成了NaN。
    ...
    def save_checkpoint(self, cpt_file_name=None, epoch=None):
        ...
        torch.save(store_dict, cpt_file_path)  # can save any project
    ...
    def resume_checkpoint(
        ...
        if torch.cuda.device_count() == 0:
            ...
            store_dict = torch.load(cpt_file_path, map_location=self.device)  # continue to run
